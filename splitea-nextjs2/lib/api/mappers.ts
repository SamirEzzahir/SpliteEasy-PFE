...
import type {
  ApiExpense, ApiFriend, ApiGroup, ApiJarBalance, ApiJarStrategy,
...
export function mapFriend(f: ApiFriend, currentUserId: number): FriendRow {
  // Pick the "other side" of the relationship
  const isOutbound = f.user_id === currentUserId;
  const otherId = isOutbound ? f.friend_id : f.user_id;
  let status: FriendRow["status"] = "friend";
  if (f.status === "pending") status = isOutbound ? "sent" : "received";
  return {
    personId: String(otherId),
    status,
    balance: 0, // balances come from /settle/global/balances and are merged later
    mutuals: 0,
    requestAt: f.created_at,
  };
}
...
export function buildJarsFromBackend(
  ...
  return JAR_ORDER.map((code) => {
    ...
    // The backend reports a signed running balance per jar. For UI purposes:
    //   - save jars: positive balance ≈ amount saved
    //   - spend jars: outflows reduce the running balance, so spent = -delta
    //     We don't have the distributed-total here; the dashboard treats
    //     remaining balance as "left" and any negative number as overdrawn.
    return {
      ...meta,
      pct: pctByCode[code],
