...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Only redirect when the request actually had a token (session expired).
    // Ignores 401s from anonymous or non-critical requests (e.g. notifications).
    const unsub = onUnauthorized((hadToken) => {
      setUser(null);
