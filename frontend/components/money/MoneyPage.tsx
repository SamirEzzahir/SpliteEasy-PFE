  ...
  return <div className="money-page">
    ...
    {current && <section id={`money-panel-${section}`} role="tabpanel" aria-labelledby={`money-${section}`} aria-busy={loading}>
      {(section==="wallets" || section==="overview") && !detailId && <div className="mw-dashboard"><div className="mw-main">
        <section className="mw-accounts"><div className="mw-balance-head"><div><p>Total balance <span>{known.length} wallets</span></p><strong className="mw-total num" data-testid="wallet-total">{money(current.summary.balance,currency)}</strong></div><button className="btn btn-ghost" onClick={()=>{setTypesOpen(true);setTypeError("");}}>Wallet types</button></div>
          {wallets.length?<><div className="mw-carousel" ref={carousel} onScroll={syncScroll} tabIndex={0} role="region" aria-label="Your wallets">{wallets.map(w=><article key={w.id} className={`mw-wallet ${walletTone(w)}`}><div className="mw-wallet-top">{walletTone(w)==="bank"?<Landmark size={24}/>:walletTone(w)==="cash"?<Banknote size={24}/>:<WalletIcon size={24}/>}<span title={w.category}>{w.category}</span></div><h2>{w.name}</h2><strong className="num">{w.needs_currency?Number(w.balance).toLocaleString():money(w.balance,w.currency)}</strong><div className="mw-wallet-bottom"><small>{w.needs_currency?"Confirm currency":w.archived_at?"Archived wallet":`Personal wallet · ${w.currency}`}</small><Link href={`/money/wallets/${w.id}`} aria-label={`View ${w.name}`}><ChevronRight size={18}/></Link></div></article>)}</div><div className="mw-carousel-controls"><span>Balances you record, kept in one place.</span><div><button disabled={scrollPosition.first} aria-label="Previous wallet" onClick={()=>carousel.current?.scrollBy({left:-320,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}><ChevronLeft size={16}/></button><button disabled={scrollPosition.last} aria-label="Next wallet" onClick={()=>carousel.current?.scrollBy({left:320,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}><ChevronRight size={16}/></button></div></div></>:empty}
