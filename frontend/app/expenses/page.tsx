...
export default function ExpensesPage() {
  ...
  // ── Mobile card renderer (fallback for hidden .exp-table on ≤768px) ──────────
  function renderExpenseCard(e: Expense) {
    const cat = categoryById(e.categoryId);
    const payer = personById(e.paidBy);
    const g = groups.find((x) => x.id === e.groupId);
    const share = myShareInfo(e, myId);
    return (
      <div key={e.id} className="gx-exp-card">
        ...
        <div className="gx-exp-card-meta">
          <div className="gx-exp-card-field">
            ...
            <span className="val">{g?.name || "—"}</span>
          </div>
          <div className="gx-exp-card-field">
          ...
          <div className="card" style={{ padding: 18 }}>
            ...
            {filtered.length === 0 ? (
                ...
                {groupedData.map(({ key, groupName, groupColor, expenses: groupExp, subtotal }) => {
                  ...
                  return (
                    <div key={`m-${key}`} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      ...
                      >
                        <Icon name="chevR" size={12} style={{ color: "var(--ink-3)", transform: collapsed ? "none" : "rotate(90deg)", transition: "transform .15s", flexShrink: 0 }} />
              ...
              <>
                ...
                {/* Mobile cards (table is hidden ≤768px) */}
                <div className="gx-exp-cards">
                  {loading
                    ? Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} className="gx-exp-card" style={{ opacity: 0.5 }}>
                          <div className="gx-exp-card-top">
                            <div className="gx-exp-card-ic sk-block" style={{ width: 42, height: 42 }} />
                            <div className="gx-exp-card-title">
                    ...
                    : paginated.map((e) => renderExpenseCard(e))}
                </div>
