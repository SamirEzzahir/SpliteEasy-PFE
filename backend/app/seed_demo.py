...
Run manually:
    ...
    python -m app.seed_demo          # seed (no-op if already seeded)
    python -m app.seed_demo --force  # wipe demo data and reseed
...
from app.models import (
    ...
    Debt, Loan, DebtRepayment, LoanRepayment, DebtLoanStatus,
)
...
# ── people ──────────────────────────────────────────────────────────────────
# (username, full name, gender). The first entry is the demo account ("you").
# NOTE: the username here is the LOGIN id — it must stay "demo" so the demo/demo
# login (and the "Try demo" button on the login page) works. The display name is
# the second field ("Samir El Amrani").
PEOPLE = [
    ("demo",    "Samir El Amrani",  "male"),
...
async def seed_demo(force: bool = False) -> None:
    async with async_session() as session:
        ...
        # ── debts (demo owes) & loans (owed to demo) ─────────────────────────
        def status_for(original: float, paid: float) -> DebtLoanStatus:
            if paid <= 0:
                return DebtLoanStatus.active
            if paid >= original - 0.01:
                return DebtLoanStatus.fully_paid
            return DebtLoanStatus.partially_paid

        # (name, original, paid, days_ago_created, days_until_due)
        debt_specs = [
            ("Hamza El Fassi", 800, 0, 40, 20),
            ("Yassine Benali", 1500, 500, 55, -5),   # overdue, partially paid
            ("Ayoub El Idrissi", 300, 300, 70, -30),  # fully paid
            ("Bank Populaire", 5000, 1250, 90, 60),
        ]
        loan_specs = [
            ("Sara Bennani", 1200, 400, 50, 15),
            ("Othmane Bennis", 600, 0, 30, 10),
            ("Zakaria Chraibi", 250, 250, 65, -20),   # fully paid
            ("Imane El Alaoui", 2000, 800, 80, 45),
        ]
        dl_count = 0
        for name, original, paid, days_ago, due_in in debt_specs:
            created = NOW - timedelta(days=days_ago)
            debt = Debt(
                user_id=demo.id, lender_name=name,
                original_amount=float(original), remaining_amount=float(original - paid),
                status=status_for(original, paid),
                due_date=NOW + timedelta(days=due_in), note=None,
                created_at=created, updated_at=created,
            )
            session.add(debt)
            await session.flush()
            if paid > 0:
                session.add(DebtRepayment(debt_id=debt.id, amount=float(paid),
                                          created_at=created + timedelta(days=3)))
            dl_count += 1
        for name, original, paid, days_ago, due_in in loan_specs:
            created = NOW - timedelta(days=days_ago)
            loan = Loan(
                user_id=demo.id, borrower_name=name,
                original_amount=float(original), remaining_amount=float(original - paid),
                status=status_for(original, paid),
                due_date=NOW + timedelta(days=due_in), note=None,
                created_at=created, updated_at=created,
            ...
            await session.flush()
            if paid > 0:
                session.add(LoanRepayment(loan_id=loan.id, amount=float(paid),
                                          created_at=created + timedelta(days=3)))
            dl_count += 1
        print(f"✅ Created {dl_count} debts & loans (active / partial / paid).")
