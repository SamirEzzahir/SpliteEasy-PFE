...
from backend.models import User, Group, Membership, Expense, Settlement, GroupMessage
from backend.schemas import GroupCreate, GroupRead, MembershipRead, GroupMessageRead
...
async def create_group(session: AsyncSession, payload: GroupCreate, current_user: User) -> GroupRead:
    ...
    from backend.repositories.activity import log_activity
...
async def get_groups(session: AsyncSession, currentuser: User) -> list[GroupRead]:
    from backend.repositories.settlement import compute_group_balances
